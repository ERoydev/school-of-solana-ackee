...
pub mod on_chain_calculator {
    ...
    pub fn addition(ctx: Context<Compute>) -> Result<()> {
        ...
        let result: i32 = operand_x
            .checked_add(operand_y)
            .ok_or_else(|| error!(CalculatorError::Overflow))?;
    ...
    pub fn division(ctx: Context<Compute>) -> Result<()> {
        ...
        if operand_y == 0 {
            return Err(error!(CalculatorError::ZeroDivision));
        }

        let result: i32 = operand_x.
            checked_div(operand_y)
            .ok_or_else(|| error!(CalculatorError::Overflow))?;
...
pub enum CalculatorError {
    ...
    WrongPrivileges,
    #[msg("You need to pass valid integers to function")]
    EmptyInput,
    #[msg("Overflow")]
    Overflow,
